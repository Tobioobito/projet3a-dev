<?php /*echo"pong";*/
	...
	if (!isset($_GET["classe"]))
	{
		...
		$req = $connexion ->prepare("SELECT * FROM sl_eleve WHERE nom_eleve LIKE ? AND eta_eleve LIKE ?");
		$req->execute(array('%'.$recherche_nom.'%', $nom_eta_master));//Array = LIKE ?
	...
	{
	$req = $connexion ->prepare("SELECT * FROM sl_eleve WHERE nom_eleve LIKE ? AND eta_eleve LIKE ? AND classe_eleve LIKE ?");
	$req->execute(array('%'.$recherche_nom.'%', $nom_eta_master, $liste_eleve_classe));//Array = LIKE ?
	}
